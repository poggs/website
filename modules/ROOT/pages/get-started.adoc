...
- Add `io.hawt:hawtio-quarkus` and the supporting Camel Quarkus extensions to the dependencies in `pom.xml` (replace `5.x.y` with the latest Hawtio release version):
+
[source,xml]
----
<dependency>
...
To embed Hawtio to an application, add `io.hawt:hawtio-embedded` to your `pom.xml` (replace `5.x.y` with the latest Hawtio release version):
